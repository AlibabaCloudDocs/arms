...
2.  在左侧导航栏选择**应用监控** \> **应用列表**，在顶部菜单栏选择目标地域，然后在**应用列表**页面右上角单击**接入应用**。

3.  在**接入应用**页面顶部复制License Key。

    ![应用监控LicenseKey](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/6570348951/p132858.png)
...
3.  在应用目录 - ack-arms-pilot页面上，在右侧的创建面板中选择目标集群，并单击**创建**。
...
2.  在左侧导航栏选择**集群**，在**集群列表**页面上的目标集群右侧**操作**列单击**详情**。
    ...
    ![Manage Cluster](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/3003700061/p53701.png)

3.  在目标集群的集群信息页面上单击**集群资源**页签，然后单击**Worker RAM角色**右侧的链接。

    ![Worker RAM Link](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/3003700061/p53704.png)
...
5.  在**策略内容**页签上单击**修改策略内容**，并将以下内容添加到**策略内容**中，最后单击**确定**。
    ...
    ![Modify RAM Authorization](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/6570348951/p53703.png)
...
5.  将arms-<yourAppName\>.ini ConfigMap配置项添加到应用Deployment的spec \> template \> spec \> containers配置下，将mountPath设置为PHP配置文件扫描目录路径。
    ...
    **说明：** 如果PHP没有配置文件扫描路径，请将arms-<yourAppName\>.ini配置项内容合并到应用的php.ini中。
    ...
    在[容器服务Kubernetes版控制台](https://cs.console.aliyun.com/#/k8s/overview)的无状态（Deployment）或有状态（StatefulSet）页面上，如果目标应用的**操作**列出现**ARMS控制台**按钮，则表示Agent安装成功。

    ![ARMS Console Button](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/5017670061/p53712.png)
...
1.  如果需要临时暂停Agent，则将[步骤](#step_ha3_6ov_alh)[5](#step_ha3_6ov_alh)添加的ConfigMap文件删除，然后再重新部署应用即可。

2.  如果需要卸载Agent，则删除[步骤](#step_y4n_031_gfg)[2](#step_y4n_031_gfg)添加的Hercules Deploy组件和[步骤](#step_ha3_6ov_alh)[5](#step_ha3_6ov_alh)添加的ConfigMap文件。
