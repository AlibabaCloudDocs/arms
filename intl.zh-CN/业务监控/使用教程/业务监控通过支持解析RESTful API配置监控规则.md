...
您的应用已接入业务监控，详情请参见[准备工作概述](/intl.zh-CN/业务监控/接入指南（Java应用）/准备工作概述.md)。
...
4.  在新建业务监控页面，**服务类型**选择**HTTP入口**，从**过滤规则**或**分组规则**下拉列表中选择**Method**。
    ...
    **说明：**
    ...
    |**入口应用**|**入口应用**列表显示所有已安装应用监控探针的Java应用，必选。选中所需应用后，ARMS自动检测其应用探针的版本号。 **说明：** 应用探针升级至2.6.2+版本才能使用业务监控功能，若检测到探针版本非2.6.2+时，请您先升级探针，具体操作，请参见[准备工作概述](/intl.zh-CN/业务监控/接入指南（Java应用）/准备工作概述.md)。
